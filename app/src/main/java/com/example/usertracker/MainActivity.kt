...
import com.example.usertracker.worker.UserReminderWorker
...
class MainActivity : AppCompatActivity() {
    ...
    private fun restoreFromBackup() {
        ...
        if (restoredUsers != null) {
            // Удаляем всех текущих пользователей и добавляем восстановленные
    ...
    private fun scheduleDailyReminder() {
        ...
        val dailyReminderRequest = PeriodicWorkRequestBuilder<UserReminderWorker>(
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val name = "Напоминание о пользователе"
            val description = "Уведомление"
    ...
    private fun testNotification() {
        val users = userAdapter.currentList
        if (users.isNotEmpty()) {
            notificationService.showRandomUserReminder(users)
        }
    }
